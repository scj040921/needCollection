...
15. **从 httpd-custom.conf 中移除对 Subversion 内存缓存大小的自定义设置**：
    VisualSVN Server 3.6 提供了一个用户界面，用于自定义 Subversion 内存对象缓存的大小，以优化服务器性能。如果您的服务器httpd-custom.conf文件被修改以更改 Subversion 内存缓存大小，这些自定义设置可能会与核心httpd.conf配置文件中的设置冲突。您应该仔细检查并移除这些自定义设置。
