...
- **全缓冲类型**：
	- 一旦填满缓冲区，立刻将数据刷新到文件
	- 程序正常退出时，立刻将数据刷新到文件
	- 遇到fflush()强制刷新时，立刻将数据刷新到文件
	- 关闭文件时立刻将数据刷新到文件中
	- 读取文件内容时，立刻将数据刷新到文件中
	- 改变缓冲区类型时，立刻将数据刷新到文件中
- **行缓冲类型**：
	- 同全缓冲类型
	- 一旦遇到换行符‘\n’，立刻将数据刷新到文件
## 关键点
1. 缓冲都是针对写操作而言的，缓冲是为了提高写效率
2. 对于标准输出而言，默认是行缓冲的
3. 对于标准输出错而言，默认是不缓冲
4. 对于普通文件而言默认是全缓冲的
5. 滞留在缓冲区中的数据被称为脏数据，脏数据的存在代表程序操作的结果与文件真实状态不一致，若未正常刷新这些脏数据就退出程序有可能会造成数据丢失
6. 缓冲区类型可以通过setbuf()修改
