...
1. 基本流程：
	1. 应用程序接收文本
	2. 调用模型生成图片
2. ImageModel的入参为包装类型ImagePrompt输出类型为ImageResponse
## 语音模型
### Text-to-Speech API概述
在SpringAI框架中，Text-to-Speech提供了一个基于OpenAI的文本转语音模型的语音端点，不仅支持多语言转换，还能满足实时语音输出的需求
# SpringAI实现RAG
## 概述
### 向量化
- **向量数据库**（Vector DataBase）是一种以数学向量的形式存储数据集合的数据库，通过一个数字列表来表示维度空间中的一个位置。在这里，向量数据库的功能是可以基于相似性搜索进行识别，而不是精准匹配。返回的信息取决于数据库中存在的数据。用户可以通过参数的设定来限定返回的情况，进而适配不同的需求。
- **嵌入模型**（Embedding Model）和向量数据库是一对亲密无间的合作伙伴，也是AI技术栈中两大紧密关联的核心组件，两者的协同作用构成了现代语义搜索、推荐系统和RAG（Retrieval Augmented Generation 检索增强生成）等应用技术的基础。
## RAG
### RAG的基本概念
RAG全称Retrieval Augmented Generation，检索增强生成，是一种结合了检索系统和生成模型的新型技术框架，其主要目的有:
 - 利用外部知识库
 - 帮助大模型生成更加准确、有依据、最新的回答
通过使用RAG，解决了传统LLM存在的两个主要问题：
 - **知识局限性**：LLM的知识被固定在训练数据中，无法知道最新消息
 - **幻觉现象**：LLM有时会编造出不存在的答案
### RAG工作流程
1. 用户输入问题：用户在shu