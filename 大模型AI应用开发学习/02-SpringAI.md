...
### RAG工作流程
1. 用户输入问题：用户在输入窗口输入自己的问题，这一数据被接收，并作为后续处理的查询入口。
2. 问题向量化：根据用户初始输入的问题，调用Embedding模型，将 问题转换为高维向量。
3. 向量数据库检索：系统会连接到一个向量数据库，然后用刚才生成的问题向量，检索数据库中与之最相似的文档片段。检索时，常见参数有：
	1. Tok-K：检索最相关的k条记录
	2. 相似度阈值：控制检索到内容的相关性
4. 构建上下文：这一阶段需要组织提示词，让LLM更好的理解背景信息。这一部分包括：
	1. 系统提示词：提前告诉LLM要遵循的行为规范，可以有效地设定模型角色，控制回答风格，防止幻觉。
	2. 构造最终输入
5. 调用LLM：将构造好的Prompt提交给LLM
    - 模型读取检索到的内容和问题
    - 组织自然、连贯、准确的回答
6. 返回最终回答给用户